...
- **Phase state machine is the source of truth.** `game.phase` drives every screen: `draft → lobby → round_open → round_closed → marking → answers → reveal → final_wager → final_open → tiebreak → done` (`answers` = the MC reads each question + answer back, cursor-paced; scores stay hidden from `marking` until `reveal` "releases" them), with `paused` orthogonal. Endpoints gate on phase (e.g. `SUBMIT_PHASES = {round_open, round_closed, final_open}` guards photo submission — `round_closed` is "pens down", teams can still hand in their sheet until `marking`). Add new phases to `VALID_PHASES` in `main.py`.
- **One question at a time.** Within a live round the host advances a cursor (`game.current_question_idx`, `POST /api/host/question` next/prev; reset to 0 when a round opens). **Anti-spoiler rule: the public round payload only ever includes questions up to the cursor** (`question_count` says how many exist); the full list appears at reveal. A loose per-question timer (`game.question_seconds`, default 60, settable via `/api/host/settings`; `question_opened_at` stamped on open/advance, elapsed computed in SQL) is a display-only cue — it never auto-advances anything.
- **Two MC modes** (`game.mc_mode`, host-switchable any time via `POST /api/host/mc-mode`, exposed in `/api/state`): **`gladys`** (default, "Gladys (AI)") — photo submissions are AI-graded into `mark` rows; **`lacey`** ("Lacey in Charge") — photos are stored but never AI-graded, a human MC enters every mark via `POST /api/host/mark` (which works with or without a submission). In gladys mode a grading exception still returns `{ok: true, graded: false}` — the submission is never lost; the MC marks it by hand.
...
- **Round building is rebuildable, with hard rules (v2).** `rounds.build_rounds()` detaches/deletes non-final rounds and rebuilds: a RANDOM `game.questions_per_person` of each contributor's questions are selected (every contributor is guaranteed ≥1 in; a contributor who submitted ≤N keeps them all). Because the pick is random, a rebuild reshuffles which of an over-cap contributor's questions are used — pass a seeded `rng` for determinism (tests do). Rounds are per-category, target 5 questions (>5 splits evenly, "History II"); sparse categories top up from same-category bank/host questions, remainders pool into "Mixed Bag" round(s). `question.source` = 'contributor' (guaranteed) | 'bank' | 'host' (filler only — never form rounds alone). **Contributors may submit at most one question per category** (`models._assert_category_unused`, enforced in add/edit/set).
...
- **Keep answers off the public display path** (one deliberate exception: during phase=`answers` the read-back exposes answer/answer_items/author for cursor-visible questions — sheets are in, marks are done). `serializers.public_question` omits answers; only host views (`host_key`) and the owner-facing `GET /api/questions/mine?contributor_id=` (so a contributor can review/edit their own) include them. Don't widen answer exposure into the player/display surface.
